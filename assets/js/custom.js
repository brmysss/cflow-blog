        ...
        return content.replace(htmlCodeBlockRegex, function(match, htmlContent) {
            ...
            if (bilibiliRegex.test(htmlContent)) {
                ...
                htmlContent = htmlContent.replace(bilibiliRegex, function(match, params) {
                    ...
                    // 确保必要参数存在
                    paramsObj.page = paramsObj.p || paramsObj.page || '1';
                    paramsObj.as_wide = '1';
                    paramsObj.high_quality = '1';
                    paramsObj.autoplay = '0';
                    
                    // 移动端特殊处理
                    if (isMobile) {
                        paramsObj.danmaku = '0'; // 禁用弹幕提高性能
                        paramsObj.as_wide = '1'; // 确保宽屏
                        paramsObj.high_quality = '1'; // 高质量
                        paramsObj.autoplay = '0'; // 禁止自动播放
                        paramsObj.is_preview = '0'; // 不是预览
                        
                        // 移除可能导致问题的参数
                        delete paramsObj.t;
                        delete paramsObj.time;
                    }
                    ...
                    console.log('优化后的参数:', newParams);
                    
                    // 使用更简洁的iframe属性，移除可能导致移动端问题的属性
                    let iframeAttributes = `src="https://player.bilibili.com/player.html?${newParams}" scrolling="no" border="0" frameborder="no" allowfullscreen="true"`;
                    
                    // 移动端使用更简单的属性集
                    if (isMobile) {
                        iframeAttributes = `src="https://player.bilibili.com/player.html?${newParams}" frameborder="0" allowfullscreen="true" width="100%" height="100%"`;
                    }
                    
                    // 构建响应式容器
                    return `<div class="video-wrapper" style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;margin:10px 0;border-radius:8px;background-color:#f5f5f5;">
                        <iframe ${iframeAttributes} style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;"></iframe>
                    </div>`;
                });
        ...
        iframes.forEach(function(iframe) {
            ...
            if (isMobile) {
                try {
                    ...
                    console.log('移动端：简化iframe属性');
                } catch (e) {
                    console.error('移动端处理视频参数失败:', e);
                }
            } else {
                // 桌面端处理
            ...
            if (parent && parent.classList.contains('video-wrapper')) {
                ...
                if (window.getComputedStyle(parent).position !== 'relative') {
                    ...
                    if (isMobile) {
                        ...
                        placeholderDiv.textContent = '点击加载B站视频';
                        
                        // 插入到iframe前面
                        parent.insertBefore(placeholderDiv, iframe);
                    }
