        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (memo.APIVersion === 'legacy') {
                ...
                content = content
                    ...
                    .replace(TEXT_NETEASE_MUSIC_REG, function(match) {
                        ...
                        if (idMatch && idMatch[1]) {
                            ...
                            if (isMobile && /Safari/i.test(navigator.userAgent) && !/Chrome/i.test(navigator.userAgent)) {
                                // 移动端Safari使用iframe
                                return `<div class="music-wrapper"><iframe frameborder="no" border="0" marginwidth="0" marginheight="0" width="330" height="86" src="//music.163.com/outchain/player?type=2&id=${musicId}&auto=0&height=66" sandbox="allow-top-navigation allow-same-origin allow-forms allow-scripts allow-popups"></iframe></div>`;
                            } else {
                                // 其他浏览器使用meting-js
                    ...
                    .replace(TEXT_QQMUSIC_REG, function(match, id) {
                        // 检测是否为移动设备
                        const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
                        if (isMobile && /Safari/i.test(navigator.userAgent) && !/Chrome/i.test(navigator.userAgent)) {
                            // 移动端Safari使用iframe
                            return `<div class="music-wrapper"><iframe frameborder="no" border="0" marginwidth="0" marginheight="0" width="330" height="86" src="//music.qq.com/miniportal/player.html?songid=${id}&auto=0" sandbox="allow-top-navigation allow-same-origin allow-forms allow-scripts allow-popups"></iframe></div>`;
                        } else {
                            // 其他浏览器使用meting-js
                            return `<meting-js auto="https://y.qq.com/n/yqq/song/${id}.html"></meting-js>`;
                        }
                    })
                ...
                parsedContent = parsedContent
                    ...
                    .replace(NETEASE_MUSIC_REG, function(match, id) {
                        // 检测是否为移动设备
                        const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
                        if (isMobile && /Safari/i.test(navigator.userAgent) && !/Chrome/i.test(navigator.userAgent)) {
                            // 移动端Safari使用iframe
                            return `<div class='music-wrapper'><iframe frameborder='no' border='0' marginwidth='0' marginheight='0' width='330' height='86' src='//music.163.com/outchain/player?type=2&id=${id}&auto=0&height=66' sandbox='allow-top-navigation allow-same-origin allow-forms allow-scripts allow-popups'></iframe></div>`;
                        } else {
                            // 其他浏览器使用meting-js
                            return `<div class='music-wrapper'><meting-js auto='https://music.163.com/#/song?id=${id}'></meting-js></div>`;
                        }
                    })
                    .replace(QQMUSIC_REG, function(match, id) {
                        // 检测是否为移动设备
                        const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
                        if (isMobile && /Safari/i.test(navigator.userAgent) && !/Chrome/i.test(navigator.userAgent)) {
                            // 移动端Safari使用iframe
